...
namespace Metrics.ElasticSearch
{
    ...
    {
        private static readonly ILog log = LogProvider.GetLogger(typeof(ElasticSearchReport));
        private static readonly string hostName = Dns.GetHostName();
        private readonly ElasticReportsConfig reportConfig;
        ...
        public ElasticSearchReport(ElasticReportsConfig reportConfig)
        {
            ...
            using (var client = new HttpClient())
            {
                try
                {
                    var json = client.GetStringAsync(nodeInfoUri).Result;
                    var nodeInfo = JsonConvert.DeserializeObject<ElasticSearchNodeInfo>(json);
                    this.replaceDotsOnFieldNames = nodeInfo.MajorVersionNumber >= 2;
                }
                catch (Exception ex)
                {
                    log.WarnException("Unable to get ElasticSearch version. Field names with dots won't be replaced.", ex);
                    this.replaceDotsOnFieldNames = false;
                }
            }
        }
        ...
        {
            switch (this.reportConfig.RollingIndexType)
            {
                case RollingIndexType.Daily:
                    return $"{this.reportConfig.Index}-{DateTime.UtcNow.ToString("yyyy-MM-dd")}";
                case RollingIndexType.Monthly:
                    return $"{this.reportConfig.Index}-{DateTime.UtcNow.ToString("yyyy-MM")}";                
                default:
                    return this.reportConfig.Index;
            }
        }
        ...
        protected override void EndReport(string contextName)
        {
            ...
            using (var client = new HttpClient())
            {
                ...
                var result = client.PostAsync(this.elasticSearchUri.ToString(), new StringContent(json)).Result;
            }
        }
        ...
        {
            return this.replaceDotsOnFieldNames ? fieldName.Replace(".", "_") : fieldName;
        }
        ...
        protected override void ReportHealth(HealthStatus status)
        {
            ...
            if (this.reportConfig.ReportingApplication != null)
            {
                props.AddRange(new[]
                {
                    new JsonProperty("AppVersion", this.reportConfig.ReportingApplication.Version),
                    new JsonProperty("UpTime", this.reportConfig.ReportingApplication.UpTime.ToString())
                });
            }
        ...
        private class ESDocument
        {
            ...
            public string ToJsonString()
            {
                ...
                return meta + Environment.NewLine + Object.AsJson(false) + Environment.NewLine;
            }
        }
    }
}